...
#   • Allows on-screen controls, and supports physical controls via Ethernet (JSON).
...
#
# TRANSPORT INPUT FORMAT (Ethernet JSON only)
# -------------------------------------------
# Arduino pushes a full JSON object every ~20 ms to port 6006 (one short-lived TCP connection per push).
# Example payload (keys used by simulator):
# {
#   "scram": true,
#   "power": true,
#   "position_read": 512,
...
# }
#
# Outbound commands to Arduino (port 5005):
#   JSON dict + newline, e.g. {"scram": true, "power": false}
#   The Arduino sets the latched states directly (absolute set, not toggle).
#
# When the simulator detects a scram condition it proactively sends the command
# so the Arduino panel latches match the simulator state.
# ------------------------------------------------------------------------------
...
import errno                      # socket error numbers (EWOULDBLOCK etc.)
import json                       # JSON telemetry / commands
...
# =========================
# ETHERNET TRANSPORT (replaces ALL serial comms)
# =========================
ARDUINO_IP = "134.50.51.22"
LISTEN_PORT = 6006                # Pi listens here – Arduino connects and pushes JSON telemetry
SEND_PORT = 5005                  # Pi connects here to send JSON commands (power / scram)

# Global listener socket (non-blocking, set up once)
server_sock = None

# Status variables (kept for header display / debug)
transport_name = "Ethernet (JSON)"
transport_rx_packets = 0
transport_tx_commands = 0
transport_last_rx_time = 0.0
transport_last_tx_time = 0.0
transport_last_error = ""

def setup_ethernet_listener():
    """One-time setup of non-blocking TCP listener on port 6006 (Arduino pushes JSON here)."""
    global server_sock, transport_name, transport_last_error
    try:
        ...
        transport_name = f"Ethernet listener (port {LISTEN_PORT})"
        transport_last_error = ""
        print(f"✅ Listening for Arduino JSON telemetry on port {LISTEN_PORT}")
        return True
    except Exception as exc:
        transport_last_error = str(exc)
        transport_name = "Ethernet setup failed"
        print(f"❌ Ethernet listener failed: {exc}")
        return False

def send_json_command(payload):
    """Send a JSON command to the Arduino on port 5005 (short-lived client connection)."""
...
def read_controls_from_ethernet():
    """Non-blocking read of one telemetry packet from the listening socket (inspired by the reference Pi example)."""
        ...
        with conn:
            ...
            if data:
                ...
                try:
                    ...
                    # Extract exactly what the simulator needs
                    position_read = payload.get("position_read", 512)
                    rod_frac = position_read / 1023.0

                    knob1 = payload.get("knob1", 512)
                    knob2 = payload.get("knob2", 512)
                    ...
                    power_sw = bool(payload.get("power", False))
                    scram_sw = bool(payload.get("scram", False))
                    ...
                    transport_last_error = f"JSON decode: {e}"
                    print(f"❌ Malformed JSON from Arduino: {decoded_raw[:300]}...")
                except Exception as e:
                    transport_last_error = str(e)
...
def start_scram(reason, now):
    ...
    if not scram_active:           # only start SCRAM once per event
        ...
        # === NEW: Send power + scram command to Arduino when a scram condition is met ===
        send_json_command({"scram": True, "power": False})
...
# MAIN LOOP
# =========================
# Setup Ethernet listener once (before the loop)
setup_ethernet_listener()
...
while running:
    ...
    # ---------- ETHERNET TELEMETRY ----------
    sc = read_controls_from_ethernet()                # attempt to read one JSON telemetry packet
    ...
    for event in pygame.event.get():                  # process Pygame event queue (for windowed mode)
        if event.type == pygame.QUIT:                 # window close button (for windowed mode)
            ...
            if POWER_RECT.collidepoint((mx, my)):     # if click POWER button
                desired_power = not power_on          # toggle intent
                sent = send_json_command({"power": desired_power})
                if not sent:
                    # Fallback standalone behavior when Arduino unreachable
                    if power_on:
            ...
            if SCRAM_RECT.collidepoint((mx, my)):     # if click SCRAM button
                if power_on and not scram_active:     # only scram if running and not already scramming
                    sent = send_json_command({"scram": True})
                    if not sent:
                        # Fallback standalone behavior when Arduino unreachable
    ...
    transport_state = "LIVE" if transport_age < 1.0 else "CONNECTED"
    transport_line = inst_font.render(
        f"Link: {transport_name}   state: {transport_state}   RX: {transport_rx_packets}   TX: {transport_tx_commands}",
        True, BLACK
    )
